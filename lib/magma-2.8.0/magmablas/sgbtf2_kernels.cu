/*
    -- MAGMA (version 2.8.0) --
       ...
       @generated from magmablas/zgbtf2_kernels.cu, normal z -> s, Thu Mar 28 12:28:14 2024
*/
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    sgbtf2_adjust_ju_fillin_kernel_batched<<<grid, threads, 0, queue->cuda_stream()>>>
    ...
    int* ju_array, magma_int_t gbstep, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    sgbtf2_swap_kernel_batched<<<grid, threads, 0, queue->cuda_stream()>>>
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t s = 0; s < batchCount; s+=max_batchCount) {
        ...
        sgbtf2_scal_ger_kernel_batched<<<grid, threads, 0, queue->cuda_stream()>>>
    ...
    magma_queue_t queue)
{
    ...
    *info  = 0;
    ...
    for(magma_int_t gbstep = 0; gbstep < n; gbstep += nb) {
        ...
        cudaError_t e = cudaLaunchCooperativeKernel((void*)sgbtf2_native_kernel, grid, threads, kernel_args, shmem, queue->cuda_stream());
    }
    ...
    magma_queue_t queue)
{
    ...
    *info  = 0;
    ...
    for(magma_int_t gbstep = 0; gbstep < n; gbstep += NB) {
        ...
        cudaError_t e = cudaLaunchCooperativeKernel((void*)sgbtf2_native_kernel_v2, grid, threads, kernel_args, shmem, queue->cuda_stream());
        if(e != cudaSuccess) {
